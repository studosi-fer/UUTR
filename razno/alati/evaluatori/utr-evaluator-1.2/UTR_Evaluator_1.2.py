...
Version 1.2
- error log fixed to work on both Python 2.7 and 3.5

How to run it?
--------------------------------------------------------------------------------

Assuming this script is named test.py
...
def get_tests():
    ...
    for item in [x for x in [os.path.join(test_directory, d) for d in os.listdir(test_directory)] if os.path.isdir(x)]:
...
def execute(tests):
    ...
    correct = 0
    template_status = "[ {:6s} ] - {:.2f}s | {}"

    print("\nTotal of {} tests will be run!\n".format(len(tests)))
    ...
    for test in tests:
        ...
        # Run program
        start = time.time()
        status = subprocess.call(cmd, shell=True, stdin=subprocess.PIPE, stdout=DEVNULL, stderr=ERROR_LOG)
        end = time.time()
